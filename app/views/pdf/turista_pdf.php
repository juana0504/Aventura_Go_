...
<body>
    ...
    <div class="page-shell">
            ...
            <table>
                <tr>
                    ...
                    <td class="hero__title">
                        ...
                        <h1 class="hero__title-main">Reporte de<br>Turistas<br>Registrados</h1>
                        <p class="hero__title-sub">Consolidado oficial de turistas registrados en plataforma</p>
                    </td>
                    <td class="hero__meta">
        ...
        <div class="intro">
            Informe ejecutivo para control administrativo de turistas, diseñado para revisión rápida, seguimiento operativo y toma de decisiones.
        </div>

    <?php
    $total = count($turistas ?? []);
    $activos = count(array_filter($turistas ?? [], fn($t) => strtolower($t['estado'] ?? '') === 'activo'));
    $inactivos = count(array_filter($turistas ?? [], fn($t) => strtolower($t['estado'] ?? '') === 'inactivo'));
    ?>
        <table class="stats">
            <tr>
                    ...
                    <?php foreach ($turistas as $turista) : ?>
                        <tr>
                            ...
                            <td class="text-center">
                                <img class="avatar" src="<?= pdf_image_data_uri('public/uploads/usuario/' . ($turista['foto'] ?? ''), 'Turista') ?>" alt="Foto turista">
                            </td>
                            <td><?= htmlspecialchars($turista['nombre'] ?? '') ?></td>
                            <td><?= htmlspecialchars($turista['genero'] ?? '') ?></td>
                            <td><?= htmlspecialchars($turista['telefono'] ?? '') ?></td>
                            <td><?= htmlspecialchars($turista['email'] ?? '') ?></td>
                            <td class="text-center"><span class="badge <?= strtolower($turista['estado'] ?? 'activo') === 'activo' ? 'badge--ok' : 'badge--soft' ?>"><?= htmlspecialchars($turista['estado'] ?? 'Activo') ?></span></td>
                        </tr>
                    <?php endforeach; ?>
                <?php else: ?>
                    <tr>
                        <td colspan="7" class="empty-row">No hay turistas registrados.</td>
                    </tr>
                <?php endif; ?>
            </tbody>
        </table>
