...
<head>
    ...
    <title>Mi Perfil — AventuraGO Hotelero</title>
    ...
    <style>
        .pv-pf-wrap { display:grid; grid-template-columns:280px 1fr; gap:24px; align-items:start; }
        .pv-pf-card { background:var(--pv-surface,#fff); border:1px solid var(--pv-border,#e2e8f0); border-radius:14px; overflow:hidden; box-shadow:0 2px 10px rgba(0,0,0,.05); text-align:center; padding-bottom:20px; }
        .pv-pf-card__banner { height:80px; background:linear-gradient(135deg,#1a2b3c 60%,#ea8217); }
        .pv-pf-avatar-wrap { margin-top:-44px; margin-bottom:12px; }
        .pv-pf-avatar { width:88px; height:88px; border-radius:50%; object-fit:cover; border:4px solid #fff; box-shadow:0 2px 8px rgba(0,0,0,.15); }
        .pv-pf-card__name { font-family:'Bebas Neue',sans-serif; font-size:1.2rem; letter-spacing:.5px; color:var(--pv-text,#1a2b3c); margin:0 0 4px; padding:0 16px; }
        .pv-pf-card__role { font-size:.78rem; color:#64748b; text-transform:uppercase; letter-spacing:.6px; margin-bottom:20px; }
        .pv-pf-nav { border-top:1px solid var(--pv-border,#e2e8f0); }
        .pv-pf-nav-btn { display:flex; align-items:center; gap:10px; width:100%; padding:13px 20px; background:transparent; border:none; border-left:3px solid transparent; text-align:left; font-size:.88rem; font-weight:600; color:#64748b; cursor:pointer; transition:all .2s; }
        ...
        .pv-pf-panel { background:var(--pv-surface,#fff); border:1px solid var(--pv-border,#e2e8f0); border-radius:14px; box-shadow:0 2px 10px rgba(0,0,0,.05); overflow:hidden; display:none; animation:pvFadeIn .35s ease-out; }
        .pv-pf-panel.active { display:block; }
        @keyframes pvFadeIn { from{opacity:0;transform:translateY(10px)} to{opacity:1;transform:translateY(0)} }
        .pv-pf-panel__header { background:#1a2b3c; color:#fff; padding:16px 24px; display:flex; align-items:center; gap:10px; font-family:'Bebas Neue',sans-serif; font-size:1.1rem; letter-spacing:1px; border-bottom:3px solid #ea8217; }
        .pv-pf-panel__header i { color:#ea8217; }
        .pv-pf-panel__body { padding:28px; }
        .pv-pf-info-grid { display:grid; grid-template-columns:1fr 1fr; gap:18px; }
        .pv-pf-info-label { font-size:.72rem; font-weight:700; text-transform:uppercase; letter-spacing:.6px; color:#94a3b8; margin-bottom:4px; }
        .pv-pf-info-value { font-size:.95rem; color:var(--pv-text,#1e293b); font-weight:500; }
        .pv-pf-label { display:block; font-size:.75rem; font-weight:700; color:#64748b; text-transform:uppercase; letter-spacing:.6px; margin-bottom:7px; }
        .pv-pf-input { width:100%; background:var(--pv-bg,#f8fafc); border:1px solid var(--pv-border,#e2e8f0); border-radius:8px; padding:11px 15px; color:var(--pv-text,#1e293b); font-size:.91rem; font-family:'DM Sans',sans-serif; transition:border-color .2s,box-shadow .2s; }
        .pv-pf-input:focus { outline:none; border-color:#ea8217; box-shadow:0 0 0 3px rgba(234,130,23,.12); background:#fff; }
        .pv-pf-pw-wrap { position:relative; }
        .pv-pf-pw-wrap .pv-pf-input { padding-right:44px; }
        .pv-pf-pw-toggle { position:absolute; right:12px; top:50%; transform:translateY(-50%); background:none; border:none; color:#94a3b8; cursor:pointer; font-size:1rem; padding:4px; }
        .pv-pf-pw-toggle:hover { color:#1a2b3c; }
        @media (max-width:900px) { .pv-pf-wrap{grid-template-columns:1fr} .pv-pf-info-grid{grid-template-columns:1fr} }
    </style>
</head>
<body class="pv-body">
<div class="pv-layout">
        ...
        <main class="pv-content">
            ...
            <div class="pv-pf-wrap">
                ...
                <div class="pv-pf-card">
                    ...
                    <p class="pv-pf-card__role">Proveedor Hotelero</p>
                    <div class="pv-pf-nav">
                    ...
                    <div class="pv-pf-panel__body">
                        <div class="pv-pf-info-grid">
                            ...
                            <div>
                                ...
                                <div class="pv-pf-info-value">Proveedor Hotelero</div>
                            </div>
                        </div>
                    </div>
                </div>
                        ...
                        <form action="<?= BASE_URL ?>proveedor_hotelero/actualizar-perfil" method="POST" enctype="multipart/form-data">
                            <div class="row g-3">
                                ...
                                <div class="col-md-6">
                                    ...
                                    <input type="text" name="nombre" class="pv-pf-input" value="<?= htmlspecialchars($usuario['nombre'] ?? '') ?>" required>
                                </div>
                                <div class="col-md-6">
                                    <label class="pv-pf-label">Correo electrónico</label>
                                    <input type="email" name="email" class="pv-pf-input" value="<?= htmlspecialchars($usuario['email'] ?? '') ?>" required>
                                </div>
                                <div class="col-md-6">
                                    <label class="pv-pf-label">Teléfono</label>
                                    <input type="tel" name="telefono" class="pv-pf-input" value="<?= htmlspecialchars($usuario['telefono'] ?? '') ?>">
                                </div>
                                <div class="col-md-6">
                                    <label class="pv-pf-label">Identificación</label>
                                    <input type="text" name="identificacion" class="pv-pf-input" value="<?= htmlspecialchars($usuario['identificacion'] ?? '') ?>">
                                </div>
                ...
                <div class="pv-pf-panel" id="panel-password">
                    ...
                    <div class="pv-pf-panel__body">
                        <form action="<?= BASE_URL ?>proveedor_hotelero/cambiar-password" method="POST">
                            <input type="hidden" name="accion" value="cambiar_password">
                            <div class="row g-3" style="max-width:480px;">
                                <div class="col-12">
                                    <label class="pv-pf-label">Contraseña actual</label>
                                    <div class="pv-pf-pw-wrap">
                                        <input type="password" id="clave_actual" name="clave_actual" class="pv-pf-input" placeholder="Contraseña actual" required>
                                        <button type="button" class="pv-pf-pw-toggle" data-target="clave_actual"><i class="bi bi-eye-fill"></i></button>
                                    </div>
                                </div>
                                <div class="col-12">
                                    <label class="pv-pf-label">Nueva contraseña</label>
                                    <div class="pv-pf-pw-wrap">
                                        <input type="password" id="clave_nueva" name="clave_nueva" class="pv-pf-input" placeholder="Nueva contraseña" required minlength="6">
                                        <button type="button" class="pv-pf-pw-toggle" data-target="clave_nueva"><i class="bi bi-eye-fill"></i></button>
                                    </div>
                                </div>
                                <div class="col-12">
                                    <label class="pv-pf-label">Confirmar contraseña</label>
                                    <div class="pv-pf-pw-wrap">
                                        <input type="password" id="confirmar" name="confirmar" class="pv-pf-input" placeholder="Repite la nueva contraseña" required minlength="6">
                                        <button type="button" class="pv-pf-pw-toggle" data-target="confirmar"><i class="bi bi-eye-fill"></i></button>
                                    </div>
                                </div>
                                <div class="col-12 mt-2">
                                    <button type="submit" class="pv-btn-primary">
